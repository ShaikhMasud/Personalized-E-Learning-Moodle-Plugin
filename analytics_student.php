...
/* ── page wrapper ── */
#sa-page { max-width: 1340px; padding: 0 0 60px; }

/* ── top bar ── */
.sa-topbar {
    display: flex; align-items: center; justify-content: space-between;
    padding: 16px 0 20px; border-bottom: 0.5px solid var(--border-md);
    margin-bottom: 20px; flex-wrap: wrap; gap: 12px;
}
.sa-topbar-left  { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
.sa-topbar-right { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }

.sa-back-btn {
    display: inline-flex; align-items: center; gap: 6px;
    font-size: 12px; font-weight: 500; color: var(--muted);
    background: var(--surface); border: 0.5px solid var(--border-md);
    border-radius: var(--rsm); padding: 6px 13px;
    font-family: var(--font); cursor: pointer;
    box-shadow: var(--sh); transition: box-shadow .15s, color .15s;
}
.sa-back-btn:hover { color: var(--text); box-shadow: var(--shm); }
...
.sa-student-name { font-size: 18px; font-weight: 600; letter-spacing: -.01em; }
.sa-student-sub  { font-size: 12px; color: var(--muted); margin-top: 1px; }
.sa-badge-pill   { font-size: 11px; font-weight: 500; font-family: var(--mono); padding: 3px 10px; border-radius: 20px; }
...
.col-left  { grid-column: 1; display: flex; flex-direction: column; gap: 14px; }
.col-mid   { grid-column: 2; display: flex; flex-direction: column; gap: 14px; }
...
.metric {
    ...
    box-shadow: var(--sh); transition: box-shadow .15s, transform .15s;
}
.metric:hover { box-shadow: var(--shm); transform: translateY(-1px); }
.metric::before {
    content: ''; position: absolute; top: 0; left: 0; bottom: 0; width: 3px;
    background: var(--accent, #D3D1C7); border-radius: var(--r) 0 0 var(--r);
}
...
.pi-card::before {
    ...
    padding: 1.5px; background: var(--pi-glow, #D3D1C7);
...
.sa-save-btn {
    ...
    border: none; border-radius: var(--rsm); transition: opacity .15s;
}
...
.lock-label { font-size: 12px; font-weight: 500; }
.lock-desc  { font-size: 10px; color: var(--hint); margin-top: 1px; }
...
.access-summary { margin-top: 14px; padding: 11px; background: var(--surface-alt); border-radius: var(--rsm); }
.access-title   { font-size: 10px; color: var(--muted); font-weight: 500; margin-bottom: 6px; text-transform: uppercase; letter-spacing: .06em; }
.access-badges  { display: flex; gap: 6px; flex-wrap: wrap; }
...
/* ── chat row ── */
.chat-row { padding: 8px 0; border-bottom: .5px solid var(--border); }
.chat-row:last-child { border-bottom: none; }
.chat-q  { font-size: 12px; line-height: 1.5; }
.chat-ts { font-size: 10px; color: var(--hint); font-family: var(--mono); margin-top: 2px; }

/* ── loading ── */
.loading { display: flex; align-items: center; gap: 7px; font-size: 11px; color: var(--muted); padding: 12px 0; }
...
<div id="sa-page">
    ...
    <div class="sa-topbar">
        <div class="sa-topbar-left">
            <button class="sa-back-btn" onclick="goBack()">← Overview</button>
            <div style="display:flex;align-items:center;gap:10px;">
                <div class="sa-avatar"><?php echo $initials ?: 'ST'; ?></div>
                <div>
                    <div class="sa-student-name"><?php echo s($name); ?></div>
                    <div class="sa-student-sub">Student ID <?php echo (int)$studentid; ?> · Teacher View</div>
                </div>
            </div>
            <span class="sa-badge-pill" id="gradePill" style="background:var(--blue-bg);color:var(--blue-txt);">Loading…</span>
        </div>
        <div style="display:flex;gap:8px;align-items:center;">
            <span style="font-family:var(--mono);font-size:10px;color:var(--hint);" id="updatedAt"></span>
    ...
    <div class="sa-body">
        ...
        <div class="col-right">
            ...
            <div class="card">
                ...
                <div class="lock-row">
                    <div>
                        <div class="lock-label" style="color:var(--strong-txt)">Easy</div>
                        <div class="lock-desc">Restrict easy-level questions</div>
                    </div>
                    <label class="sa-toggle">
                        <input type="checkbox" id="lock-easy" onchange="toggleLock('easy', this.checked)">
                        <div class="sa-toggle-bg"></div>
                        <div class="sa-toggle-knob"></div>
                    </label>
                </div>
                <div class="lock-row">
                    <div>
                        <div class="lock-label" style="color:var(--avg-txt)">Medium</div>
                        <div class="lock-desc">Restrict medium-level questions</div>
                    </div>
                    <label class="sa-toggle">
                        <input type="checkbox" id="lock-medium" onchange="toggleLock('medium', this.checked)">
                        <div class="sa-toggle-bg"></div>
                        <div class="sa-toggle-knob"></div>
                    </label>
                </div>
                <div class="lock-row">
                    <div>
                        <div class="lock-label" style="color:var(--weak-txt)">Hard</div>
                        <div class="lock-desc">Restrict hard-level questions</div>
                    </div>
                    <label class="sa-toggle">
                        <input type="checkbox" id="lock-hard" onchange="toggleLock('hard', this.checked)">
                        <div class="sa-toggle-bg"></div>
                        <div class="sa-toggle-knob"></div>
                    </label>
                </div>
                <div class="access-summary">
                    ...
                    <div class="access-badges" id="accessBadges"><div class="loading"><div class="spinner"></div></div></div>
                </div>
            </div>

            <!-- Student chats -->
            <div class="card">
                <div class="card-title">💬 Student chats</div>
                <div id="chatList"><div class="loading"><div class="spinner"></div></div></div>
            </div>
...
function statusOf(p) {
    if (p===null) return {label:"Not Attempted",cls:"bm",color:"#A8A8A8"};
    if (p<40)     return {label:"Weak",         cls:"bw",color:"#E24B4A"};
    if (p<70)     return {label:"Average",       cls:"ba",color:"#EF9F27"};
    return              {label:"Strong",         cls:"bs",color:"#639922"};
}
function gradeOf(a) {
    if (a>=90) return {l:"A+",bg:"#EAF3DE",c:"#3B6D11"};
    if (a>=80) return {l:"A", bg:"#EAF3DE",c:"#3B6D11"};
    if (a>=70) return {l:"B", bg:"#E8F2FC",c:"#185FA5"};
    if (a>=60) return {l:"C", bg:"#FEF3E0",c:"#854F0B"};
    if (a>=50) return {l:"D", bg:"#FEF3E0",c:"#854F0B"};
    return          {l:"F", bg:"#FCEBEB",c:"#A32D2D"};
}
function unitPct(u) {
    const d=TOPIC_DATA[u], s=d.easy.s+d.medium.s+d.hard.s, t=d.easy.t+d.medium.t+d.hard.t;
    return t>0?Math.round(s/t*100):null;
}
function scorePillCls(s,t) {
    if(!t) return 'score-mid';
    const p=s/t; return p>=0.7?'score-high':p>=0.4?'score-mid':'score-low';
}
function diffBadge(d) {
    const dl=(d||'').toLowerCase();
    return dl==='easy'?`<span class="badge bs">Easy</span>`:dl==='medium'?`<span class="badge ba">Medium</span>`:dl==='hard'?`<span class="badge bw">Hard</span>`:`<span class="badge bm">${d||'—'}</span>`;
}
...
function renderMetrics() {
    ...
    document.getElementById('gradePill').style.cssText=`background:${g.bg};color:${g.c};border:.5px solid ${g.c}44;font-family:var(--mono);font-size:11px;font-weight:500;padding:3px 10px;border-radius:20px;`;
...
function loadLocks() {
    post('get_locks').then(data=>{
        ...
        renderAccessBadges();
    }).catch(renderAccessBadges);
}
function toggleLock(difficulty,isLocked) {
    post('toggle_lock',{difficulty,locked:isLocked?1:0}).then(renderAccessBadges);
}
function renderAccessBadges() {
    const diffs=[
        {id:'lock-easy',  label:'Easy',  bg:'#EAF3DE',c:'#3B6D11'},
        {id:'lock-medium',label:'Medium',bg:'#FEF3E0',c:'#854F0B'},
        {id:'lock-hard',  label:'Hard',  bg:'#FCEBEB',c:'#A32D2D'},
    ];
    document.getElementById('accessBadges').innerHTML=diffs.map(d=>{
        const locked=document.getElementById(d.id)?.checked;
        return `<span class="badge" style="background:${d.bg};color:${d.c};border:none">${d.label} ${locked?'✗':'✓'}</span>`;
    }).join('');
}
...
function loadChats() {
    post('get_student_chat').then(data=>{
        ...
        if(!data||!data.length){list.innerHTML=`<p style="font-size:11px;color:var(--hint)">No chats found.</p>`;return;}
        list.innerHTML=data.map(c=>`<div class="chat-row"><div class="chat-q"><strong style="color:var(--blue-txt)">Q:</strong> ${c.question||c.message||'—'}</div><div class="chat-ts">${fmtDate(c.timecreated)}</div></div>`).join('');
    }).catch(()=>{document.getElementById('chatList').innerHTML=`<p style="font-size:11px;color:var(--hint)">Could not load chats.</p>`;});
}
